...
class OpenImagesDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.classname = pd.read_csv(
            classname_file, header=None, names=["LabelName", "Description"]
        )
        ...
        self.logger.info(
            "Filtering Annotations by subset: {}".format(filter_subset)
        )
        ...
        if self.fast_init_slow_train:
            self.logger.info(
                "### CAUTION. DEBUGGING PURPOSES --> fast init slow train ###"
            )
            self.ids = self.detections_ann["ImageID"].unique()
        else:
            self.logger.info("Converting Pandas to Dict of Pandas")
            self.annotations = {}
            for groundtruth in tqdm.tqdm(
                self.detections_ann.groupby("ImageID")
            ):
    ...
    def __getitem__(self, idx):
        ...
        if self.fast_init_slow_train:
            anno = self.detections_ann[
                self.detections_ann["ImageID"] == image_id
            ]
        ...
        if len(anno) > 500:
            ...
            for l in label_type_in_image:
                labels_to_delete = labels_to_delete.union(
                    self.parents_hierarchy[l]
                )
        ...
        if self.use_image_labels:
            ...
            for c in classes:
                image_classes_positive = image_anno[
                    image_anno["Confidence"] == 1
                ]["LabelName"]
                image_classes_positive = [
                    self.json_category_id_to_contiguous_id[cc]
                    for cc in image_classes_positive
                ]
                if len(image_classes_positive) == 0:
                    image_classes_positive = [0]
                image_classes_positive = torch.tensor(
                    image_classes_positive, dtype=torch.int64
                )

                image_classes_negative = image_anno[
                    image_anno["Confidence"] == 0
                ]["LabelName"]
                image_classes_negative = [
                    self.json_category_id_to_contiguous_id[cc]
                    for cc in image_classes_negative
                ]
                if len(image_classes_negative) == 0:
                    image_classes_negative = [0]
                image_classes_negative = torch.tensor(
                    image_classes_negative, dtype=torch.int64
                )

                list_target_positive.append(image_classes_positive)
                list_target_negative.append(image_classes_negative)
            target.add_field(
                "image_labels_positive", torch.stack(list_target_positive)
            )
            target.add_field(
                "image_labels_negative", torch.stack(list_target_negative)
            )
    ...
    def _do_filter_annotations_group_by_count(self, pd1, pd2, params):
        self.logger.info(
            "Filter by count {} to {} [input: {}]".format(
                params[0], params[1], pd1.shape[0]
            )
        )
        counts = (
            pd1.groupby("LabelName")
            .count()
            .sort_values("ImageID")
            .reset_index()[["LabelName", "ImageID"]]
        )
        labels_to_use = counts[int(params[0]) : int(params[1])][
            "LabelName"
        ].values
        ...
        self.logger.info(
            "Filter by count {} to {} [output: {}]".format(
                params[0], params[1], pd1.shape[0]
            )
        )
        return pd1, pd2
    ...
    def _do_filter_annotations_group_by_topic(self, pd1, pd2, params):
        self.logger.info(
            "Filter by topic [#topics:{}. select:{}] [input: {}]".format(
                params[0], params[1], pd1.shape[0]
            )
        )
        pd1["count"] = 1
        ...
        temp_label_to_id = {v: i for i, v in enumerate(self.categories)}
        temp_id_to_label = {v: k for k, v in temp_label_to_id.items()}
        self.logger.info("-> Creating co-occurrence matrix")
        for image_id, item in tqdm.tqdm(
            counts.reset_index().groupby("ImageID")
        ):
            ...
            for i in labels_in_image:
                for j in labels_in_image:
                    if temp_label_to_id[i] != temp_label_to_id[j]:
                        cooccurrences[
                            temp_label_to_id[i], temp_label_to_id[j]
                        ] += 1
        ...
        kmeans = KMeans(n_clusters=int(params[0]), random_state=0).fit(
            cooccurrences
        )
        ...
        for i in range(int(params[0])):
            temp_labels_to_use = [
                str(temp_id_to_label[l])
                for l in np.where(kmeans.labels_ == i)[0]
            ]
            n_images = pd1[pd1["LabelName"].isin(temp_labels_to_use)].shape[0]
            self.logger.info(
                "      topic:{} -> {} labels ({} images)".format(
                    i, np.sum(kmeans.labels_ == i), n_images
                )
            )

        labels_to_use = [
            str(temp_id_to_label[l])
            for l in np.where(kmeans.labels_ == int(params[1]))[0]
        ]

        pd1 = pd1[pd1["LabelName"].isin(labels_to_use)]
        pd2 = pd2[pd2["LabelName"].isin(labels_to_use)]
        self.logger.info(
            "Filter by topic [#topics:{}. select:{}] [output: {}]".format(
                params[0], params[1], pd1.shape[0]
            )
        )
        return pd1, pd2
    ...
    def filter_annotations(self, type_filtering):
        ...
        if type_filtering == "no_group_of":
            self.detections_ann = self._do_filter_annotations_group_of(
                self.detections_ann
            )
        elif type_filtering == "leaves":
            self.detections_ann = self._do_filter_annotations_leaves(
                self.detections_ann
            )
            ...
            self.detections_ann, self.image_ann = self._do_filter_annotations_group_by_count(
                self.detections_ann, self.image_ann, parameters[1:]
            )
        elif "groupByTopic" in type_filtering:
            parameters = type_filtering.split("_")
            self.detections_ann, self.image_ann = self._do_filter_annotations_group_by_topic(
                self.detections_ann, self.image_ann, parameters[1:]
            )
        else:
